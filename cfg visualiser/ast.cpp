        ...
        if (Declaration->hasBody())
        {
            ...
            if (cfg)
            {
                ...
                for (auto *Block : *cfg)
                {
                    ...
                    {
                        color = "lightgray";
                        status = "!!! UNREACHABLE (Optimizable) !!!\\n";
                    }
                    else if (analyzer.hasBug(id))
                    {
                        color = "red";
                        status = "!!! POTENTIAL UNINIT BUG !!!\\n";
                    }

                    dotFile << "    " << nodeName << " [label=\"Block " << id << "\\n"
                            << status;
                    dotFile << "IN: { ";
                    for (int rdaId : analyzer.getInSet(id))
                        dotFile << "d" << rdaId << " ";
                    dotFile << "}\\n----------------\\n";
                    ...
                    dotFile << "----------------\\nOUT: { ";
                    for (int rdaId : analyzer.getOutSet(id))
                        dotFile << "d" << rdaId << " ";
                    dotFile << "}\", style=filled, fillcolor=" << color << "];\n";

                    for (auto &succ : Block->succs())
                        if (succ)
                            dotFile << "    " << nodeName << " -> " << funcName << "_Node" << succ->getBlockID() << ";\n";
                }
                dotFile << "  }\n";
            }
        }
